...
void loop() {
  ...
  analogWrite(DAC1, int(k*waveformsTable[wave1][i]));  // write the selected waveform on DAC1

  i++;
  if(i == maxSamplesNum)  // Reset the counter to repeat the wave
    i = 0;

  delayMicroseconds(80000/sample);  // Hold the sample value for the sample time
}
